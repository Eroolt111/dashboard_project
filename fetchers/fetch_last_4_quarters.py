...
def fetch_last_4_quarters(tbl_id, code_list=None):
    ...
    last_4_sorted = sorted(last_4)  # ascending for API call

    # Convert to string format: "YYYYQQ"
    last_4_str = [f"{y}{q:02d}" for (y, q) in last_4_sorted]
    ...
    print(f"✅ Found {len(filtered_records)} records for quarters: {', '.join(last_4_str)}")
